...
public class LocationConverter {
	...
	public static Location convertPhysicalLocationToLocationResource(PhysicalLocation physicalLocation) {
		...
		if (StringUtils.isNotBlank(physicalLocation.getProperties().getType())) {
			builder.type(CodeableConcept.builder().id("locationType")
			        .text(String.of(physicalLocation.getProperties().getType())).build());
		}
		...
		if (physicalLocation.getServerVersion() != null) {
		...
		builder.status(locationStatus).partOf(partOf).identifier(identifiers).mode(mode).physicalType(physicalType);
